Ractive.components.vScroll = Ractive.extend({
	template: '<div class="virtual-scroller" id="{{id}}" on-touchmove="scroll"> {{#if reverse && blocks.length - render.length > 0}} <span on-click="add:top" class="vscroll-loader-top">Previous</span> {{/if}} {{#each render:i}} <div class="v-block" on-viewport="visible:{{this}}" id="{{id}}-{{this}}"> {{#if visible.indexOf(this) > -1}} {{#each range(this, blockCount, list.length, reverse, blocks):j}} {{#if list[this]}} {{>formatter list[this]}} {{/if}} {{/each}} {{/if}} </div> {{/each}} {{#unless reverse}} <div class="vscroll-loader-bottom" on-viewport="add:bottom"></div> {{/unless}} </div>',
	init: function (options) {
		this.observe('list', function(){
			var data = this.data, len = data.list.length, count = data.blockCount, rev = data.reverse;
		...
		var element = document.getElementById(this.data.id), timer;
		var ADD_TOP = "top";
